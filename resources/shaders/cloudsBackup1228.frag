...
float Attenuation( float density_to_sun,float cos_angle,float attuention_CampVal)
{
	...
	//float cloud_beer = exp(-density_to_sun*precipiFactor);
	//float prim =remap(exp(-cloud_beer*density_to_sun),2,0,0,1.5f);//this is ugly but usefull
	...
	//return mix(checkval,prim,0.5);//simulate the clouds reduce brightness when face to sun
...
// beer+powder+HG
float getLightEnergy(float lightDotEye,float densityToSun,float cloudDensity,float precipitation,float percent_height,float stepSize,float lowLoddedDensity,float phase)
{

	///////////////////////Easy Way
//	float beerLaw = exp(-densityToSun*precipiFactor);
//	
//	
//	float powder = powder(cloudDensity, lightDotEye) ;
//	float DarknessThreshold =0.77;
//	powder =  DarknessThreshold + powder * (1.0 - DarknessThreshold);
...
//
//	2022-12-20:
//	   float depth_probability = lerp( 0.05 + pow(lowLoddedDensity, remap( percent_height, 0.1, 0.75, 0.5, 2.0 )), 1.0, saturate( densityToSun / stepSize));
//
//		float vertical_probability = pow( remap( percent_height,0.05, 0.14, 0,1 ), 0.95);//0.95
//
//	*/
//
...
//	float attenuation_probability =  max( exp( -densityToSun*precipiFactor),exp(-densityToSun * 0.25) * 0.7);
//	//return attenuation_probability;
//   float depth_probability = lerp( 0.12 + pow(lowLoddedDensity, remap( percent_height, 0.1, 0.75, 0.5, 2.0 )), 1.0, saturate( densityToSun / stepSize));
//
//	float vertical_probability = pow( remap( percent_height,0.02, 0.15, 0,1 ), 0.85);//0.95
//	 float in_scatter_probability = depth_probability * vertical_probability;
//	 //return pow(lowLoddedDensity,2.0);
//	 //return phase;
//	 float eccentricity =      0.6;
//	 float silver_intensity =  7;
//	 float silver_spread =     0.85;
//	 float Energy = max(HG(lightDotEye, eccentricity), silver_intensity* HG(lightDotEye, 0.99-silver_spread));
//
//	 float powderSugar = 1.0-exp(-densityToSun * 2.0);
//	 float Atten =  Attenuation(densityToSun,lightDotEye,0.2f);
//	 //return powderSugar;
//	// float Energy = InOutScatter(lightDotEye,5.0f,5.0f,1f);
//	// Energy = 5*HG(lightDotEye,0.5);
//	//float sun_highlight = FlatCloudHg(lightDotEye);
//	//	return depth_probability;
//	
//	 Energy = HG(lightDotEye, 0.7);
//
//	// return in_scatter_probability;
//	//return Atten*powderSugar*Energy;
//
//	////////////////////////////
	...
	float secondary_attenuation = exp(-densityToSun * 0.25) * 0.7;
	float attenuation_probability = max(primary_attenuation,secondary_attenuation);
	// 0.7 1.0 0.9												↓circle's size &softness
	//float attenuation_probability = max( remap( lightDotEye, 0.945, 0.95, secondary_attenuation, secondary_attenuation*0.9/*this value is the contrast between face sun area and other area*/) , primary_attenuation);
	//float attenuation_probability = max( remap( lightDotEye, 0.7, 1.0, secondary_attenuation, secondary_attenuation*0.25/*this value is the contrast between face sun area and other area*/) , primary_attenuation);
	
	//return attenuation_probability;

	float attenuation_prob = Attenuation(densityToSun,lightDotEye,0.25f);//beer LAMBERT lAw  BBS
	//////////////////////////////
	//SCATTER
	//////////////////////////////
	//SIG2017
	   float depth_probability = lerp( 0.08 + pow(cloudDensity, remap( percent_height, 0.1, 0.75, 0.5, 2.0 )), 1.0, saturate( densityToSun / stepSize));

		float vertical_probability = pow( remap( percent_height,0.05, 0.15, 0,1 ), 0.95);//0.95
	 float in_scatter_probability = depth_probability * vertical_probability;

	//BBS
	float ambient_out_scatter = OutScatterAmbient(lowLoddedDensity,percent_height);
	//////////////////////////////
	//highLight
	/////////////////5////////////	
	//Can be calculated once for each march but gave no/tiny perf improvements.
	float eccentricity =      0.6;
	float silver_intensity =  11;
	float silver_spread =     0.5;
	float Energy = max(HG(lightDotEye, eccentricity), silver_intensity* HG(lightDotEye, 0.1));
	//return Energy;
	float sun_highlight = InOutScatter(lightDotEye,4.5f,2f,0.5f);
	
	
	//////////////////////////////
	//AMBIENT
	//////////////////////////////
	float attenuation = attenuation_probability*in_scatter_probability*Energy*sunEnergy;

	float cloud_ambient_minimum =0.5;
	attenuation = max(cloudDensity*cloud_ambient_minimum*(1 -pow(saturate(densityToSun/4000),2)),attenuation);
	///////////////////////////
	//return
	///////////////////////////
	//return in_scatter_probability;
...
//	vec3 lightStep = stepSize * lightDir;
	...
	for(uint i = 0u; i < LIGHT_RAY_ITERATIONS; ++i)
	{
		...
		return saturate(getLightEnergy(lightDotEye,densityToSun,cloudDensity, 
	...
	vec4 blueNoise)			// world space end position
{
	...
	//second,if the cloudDensity>0,calculate the density from
		if(cloudDensity > 0.0f)
		{
			...
			lightDensity = sampleClouddensityToSun(
				...
				cloudDensity,
				lowLoddedDensity,
				phase);//返回的就是云层的灯光强度值[0,1]
			//TODO 
			 ambientBadApprox = ambientLight(heightFrac) * min(1.0f, length(sunColor.rgb * 0.0125f)) * transmittance;
			 vec3 ambientLight = ambientLight(heightFrac);
			 vec4 source = vec4((sunColor.rgb * lightDensity +ambientLight*anbientIntensity*AmbientColor(heightFrac))/*+ ambientLight(heightFrac)*/, cloudDensity * transmittance); 
	...
	//fogAmt = 0 ;//to disable
	vec3 fogColor = vec3(0.4f, 0.54f, 0.6f) * length(sunColor.rgb * 0.825f) * 0.8f;
...
//	vec3 posW = mul(vec4(posV, 1.0f), inverseViewMatrix).xyz; //世界空间下的片元坐标 用深度图算出来的世界坐标
