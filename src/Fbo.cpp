...
void Fbo::GetColTexFbo(GLuint& FBO, GLuint& texture, GLsizei width, GLsizei height)
{
...
void Fbo::getShdowFboMap(GLuint &shadowMapFBO, GLuint &shadowMap, GLsizei SMWidth, GLsizei SMHeight)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, SMWidth, SMHeight, 0, GL_DEPTH_COMPONENT,
		GL_FLOAT, NULL);
...
void Fbo::getIndirectDiffuseMaps(GLuint* cubemapFromHDR, GLuint* irradianceMap, Shader* equalRectTOCubeShader, Shader* irradianceMapShader)
{
	...
	equalRectTOCubeShader->PassUniformToShader("projection", captureProjection);
	...
	for (unsigned int i = 0; i < 6; ++i)
	{
		equalRectTOCubeShader->PassUniformToShader("view", captureViews[i]);
	...
	irradianceMapShader->PassUniformToShader("projection", captureProjection);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, *cubemapFromHDR);
	...
	for (unsigned int i = 0; i < 6; ++i)
	{
		irradianceMapShader->PassUniformToShader("view", captureViews[i]);
...
// -------------------------------------------------------------
void Fbo::getIndirectSpecularMaps(GLuint* prefilterMap, GLuint* brdfLUTTexture, GLuint* cubemapFromHDR,Shader* preFilterShader,Shader* genBrdfLutShader)
{
	...
	preFilterShader->PassUniformToShader("projection", captureProjection);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, *cubemapFromHDR);
	...
	for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
	{
		...
		for (unsigned int i = 0; i < 6; i++)
		{
			preFilterShader->PassUniformToShader("view", captureViews[i]);
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	Mesh::drawQuad();
	glViewport(0, 0, m_screenHeight, m_screenHeight);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
};
