...
float ShadowCalculation(vec4 fragPosLightSpace)
{
    ...
    float bias = max(shadowMapBias * (1.0 - dot(normalDir, lightDir)), shadowMapBias);
    
   
    for (int x = -SPFSize; x < SPFSize; ++x)
    {
        for (int y = -SPFSize; y < SPFSize; ++y)
        {
...
//        vec3 radiance = lightColors * attenuation;
  ...
  // FragColor =distance(camPos-WorldPos);
