...
namespace Render
{
    void OpenGL_VertexIndexBuffer::create_buffers(const std::vector<Base::VertexHolder>& vertices, const std::vector<unsigned int>& indices)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(Base::VertexHolder), vertices.data(), GL_STATIC_DRAW);

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mIBO);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(GLuint), indices.data(), GL_STATIC_DRAW);

        glEnableVertexAttribArray(0);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Base::VertexHolder), (void*)0);

        glEnableVertexAttribArray(1);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Base::VertexHolder), (void*)offsetof(Base::VertexHolder, mNormal));
