...
namespace Gui
{
	void Scene::resize(int32_t width, int32_t height)
	{
		...
		m_FrameBuffer->create_buffers((int32_t)m_Size.x, (int32_t)m_Size.y);
	}

	void Scene::on_mouse_move(double x, double y, Base::InputButton button)
	{
	...
	void Scene::load_mesh(const std::string& filepath)
	{
		if (!m_Mesh)
			m_Mesh = std::make_shared<Base::Mesh>();
...
// vertex shader
		...
		ImGui::Image(reinterpret_cast<void*>(textureID), ImVec2{m_Size.x, m_Size.y});
